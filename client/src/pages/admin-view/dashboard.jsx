...
function AdminDashboard() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6 md:p-8 flex flex-col gap-6">
      {/* Upload Section */}
      <ProductImageUpload
        imageFile={imageFile}
        setImageFile={setImageFile}
      ...
      <Button
        ...
        className="mt-4 w-full sm:w-48 self-start"
      >
        Upload
      </Button>
        ...
        {localBanners.map((banner, index) => (
          <div key={index} className="relative w-full">
            <img
              src={banner.image}
              alt={`banner-${index}`}
              className="w-full h-[30vh] sm:h-[40vh] md:h-[50vh] lg:h-[600px] xl:h-[700px] object-cover rounded-lg shadow-md"
            />
          </div>
        ))}
      </div>
